...
def query_02(dataset_path, scale):
    ...
    nation_filtered = dd.read_csv(f'{dataset_path}/nation.csv')
    supplier_filtered = dd.read_csv(f'{dataset_path}/supplier.csv')
    part_filtered = dd.read_csv(f'{dataset_path}/part.csv')
    partsupp_filtered = dd.read_csv(f'{dataset_path}/partsupp.csv')
...
def query_08(dataset_path, scale):
    ...
    orders["o_year"] = dd.to_datetime(orders["o_orderdate"]).dt.year
    total = total.merge(
...
def query_09(dataset_path, scale):
    ...
    subquery["o_year"] = dd.to_datetime(subquery.o_orderdate).dt.year
